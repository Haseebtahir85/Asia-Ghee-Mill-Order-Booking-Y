...
// - Full catalog list (every item, blank if not ordered on this order —
//   not just the lines actually ordered), in TWO panels per bill: a
//   customer-facing panel (Item / Qty / Rate / Amount / Type /
//   Weight (Ton)) and a dispatch/loading panel (Item / Type / Qty /
//   Dispatched, left blank for hand-checkoff).
// - A summary section below with real computed totals — Weight (Ton) &
//   Amount on one line, then the six-way breakdown (Weight (Ghee/Oil),
//   Total Weight (Ton), Weight (RSO/SOAP), G.Total Weight (Ton)) laid
//   out as a column-divided stat bar, same visual pattern as the
//   booking page's weight breakdown (label above, divider, value below,
//   vertical dividers between columns) rather than stacked lines.
// - TWO bills stacked on one A4 landscape page (top half / bottom half).
//   With an odd number of orders, the last page has one bill and a
//   blank bottom half.
//
// Given the full ~30-row catalog printed twice (two panels) per bill,
// and two bills per page, row height is necessarily very small (~6pt
// font). If this prints too small to read comfortably, the fallback is
// one bill per page — say so and it's a one-line change.
...
export function buildOrderBookPdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    function drawBill(order: any, top: number) {
      ...
      // --- Header ---
      doc.font("Helvetica-Bold").fontSize(9).text("ASIA GHEE MILLS (Pvt.) Ltd.", left, y, { width: pageWidth, align: "center" });
      y += 11;
      doc.fontSize(8.5).fillColor("#0b2b5b").text(`Order #: ${order.order_number}`, left, y, { width: pageWidth, align: "center" });
      doc.fillColor("#000");
      y += 10;
      doc.font("Helvetica").fontSize(6.5).fillColor("#555").text(`Town: ${order.town ?? ""}    Date: ${order.order_date}`, left, y, { width: pageWidth, align: "center" });
      doc.fillColor("#000");
      y += 9;
      ...
      const totalWeightAllTon = gheeTon + oilTon + rsoTon + soapTon; // == grandTotalTon

      y += 2;
      doc.font("Helvetica-Bold").fontSize(6.2);
      doc.text(`Weight (Ton): ${totalWeightAllTon.toFixed(3)}`, leftX0, y, { width: leftTotalW / 2 });
      doc.text(`Amount: ${Math.round(totalAmount).toLocaleString()}`, leftX0 + leftTotalW / 2, y, { width: leftTotalW / 2, align: "right" });
      y += 9;

      // --- Column-divided stat bar (booking-page style) ---
      const stats = [
      ...
      const barWidth = leftTotalW + gap + rightTotalW;
      const cellW = barWidth / stats.length;
      const barTop = y;
      const barHeight = 14;
      doc.rect(leftX0, barTop, barWidth, barHeight).strokeColor("#0b2b5b").lineWidth(0.5).stroke();
      stats.forEach((s, i) => {
        const cx = leftX0 + i * cellW;
        if (i > 0) doc.moveTo(cx, barTop).lineTo(cx, barTop + barHeight).strokeColor("#cfd6e4").lineWidth(0.5).stroke();
        doc.font("Helvetica-Bold").fontSize(4.6).fillColor("#666").text(s.label, cx + 2, barTop + 1.5, { width: cellW - 4, align: "center" });
        doc.font("Helvetica-Bold").fontSize(6.5).fillColor("#0b2b5b").text(s.value, cx + 2, barTop + 7, { width: cellW - 4, align: "center" });
      });
      doc.fillColor("#000");
    }
    ...
    for (let i = 0; i < orders.length; i += 2) {
      if (i > 0) doc.addPage();
      drawBill(orders[i], doc.page.margins.top);
      if (orders[i + 1]) {
        drawBill(orders[i + 1], doc.page.margins.top + halfHeight);
      }
    }
